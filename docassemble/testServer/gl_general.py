...
from docassemble.base.util import Address, Individual, DAObject, DAList, date_difference, name_suffix, states_list, comma_and_list, word, comma_list, url_action, get_config, phone_number_is_valid, validation_error, DAWeb, get_config, as_datetime, DADateTime, subdivision_type
...
class GLAddressList(DAList):
  """Store a list of Address objects"""
  def init(self, *pargs, **kwargs):
    super(GLAddressList, self).init(*pargs, **kwargs)
    self.object_type = GLAddress

  def __str__(self):
    return comma_and_list([item.on_one_line() for item in self])

class GLPeopleList(DAList):
  """Used to represent a list of people. E.g., defendants, plaintiffs, children"""
  def init(self, *pargs, **kwargs):
    super(GLPeopleList, self).init(*pargs, **kwargs)
    self.object_type = GLIndividual
...
class GLIndividual(Individual):
  ...
  and `other_addresses`
  """
  previous_addresses: GLAddressList
  other_addresses: GLAddressList
  mailing_address: GLAddress

  def init(self, *pargs, **kwargs):
    super(GLIndividual, self).init(*pargs, **kwargs)
    ...
    self.reInitializeAttribute('address',GLAddress)
    
    if not hasattr(self, 'previous_addresses'):
      self.initializeAttribute('previous_addresses', GLAddressList)
    if not hasattr(self, 'other_addresses'):
      self.initializeAttribute('other_addresses', GLAddressList)
    if not hasattr(self, 'mailing_address'):
      self.initializeAttribute('mailing_address',GLAddress)
  ...
  def name_fields(self, person_or_business:str = 'person') -> List[Dict[str, str]]:
    ...
    if person_or_business == 'person':
      ...
      show_if_indiv = {"variable": self.attr_name("person_type"), "is": "GLIndividual"}
      ...
      return [
        {"label": str(self.person_type_label), "field": self.attr_name('person_type'),
         "choices": [{str(self.individual_choice_label): "GLIndividual"},
...
class VCIndividual(GLIndividual):
  pass

class AddressList(GLAddressList):
  pass

class PeopleList(GLPeopleList):
  pass
