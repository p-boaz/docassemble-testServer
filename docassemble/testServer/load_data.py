...
__all__ = ['DataLoader', 'conditions_with_help', 'ConditionsDict', 'DataCatDict', 'datacats_with_help', 'tr_category']
...
class DataCat(DAObject):
  def init(self, *pargs, **kwargs):
    super().init(*pargs, **kwargs)
    self.initializeAttribute('details', DADict)
    self.details.object_type = DAObject

class DataCatDict(DADict):
  def init(self, *pargs, **kwargs):
    super().init(*pargs, **kwargs)
    self.object_type = DataCat
    self.complete_attribute = 'complete'
    
  def count_datacats(self):
    """Return the total number of selected datacats"""
    total = 0
    if not len(self):
      return 0
    for category in self:
  ...
  def as_merged_list(self):
    """Merge datacat details with original DF row"""
    results = pd.concat([self[c].df for c in self])

def datacats_with_help(dataloader: DataLoader, category:str, search_column:str='Category', language="en")->List[Dict]:
  """
  Function that simplifies grabbing the row, interview description, and full description given
  a specific category.
  """
